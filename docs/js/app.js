...
var _lovefield = __webpack_require__(125);
...
function schema() {
  ...
  schemaBuilder.createTable('Item').addColumn('id', _lovefield.Type.INTEGER).addColumn('name', _lovefield.Type.STRING).addColumn('edit', _lovefield.Type.STRING).addColumn('complete', _lovefield.Type.BOOLEAN).addColumn('editing', _lovefield.Type.BOOLEAN).addColumn('new', _lovefield.Type.BOOLEAN).addPrimaryKey(['id'], true);
  ...
  var _ref = _asyncToGenerator( /*#__PURE__*/regeneratorRuntime.mark(function _callee2() {
    ...
    var updateDomvm = function () {
      var _ref2 = _asyncToGenerator( /*#__PURE__*/regeneratorRuntime.mark(function _callee(changes) {
        var newTask, all, active, completed, results;
        return regeneratorRuntime.wrap(function _callee$(_context) {
          while (1) {
            switch (_context.prev = _context.next) {
              case 0:
                newTask = newTaskQuery.exec();
                all = allQuery.exec();
                active = activeQuery.exec();
                completed = completedQuery.exec();
                _context.next = 6;
                return newTask;
    ...
    var item, notNew, newTaskQuery, allQuery, activeQuery, completedQuery, empty, vm;
    return regeneratorRuntime.wrap(function _callee2$(_context2) {
      while (1) {
        switch (_context2.prev = _context2.next) {
          case 0:
            _context2.next = 2;
            return (0, _schema.initDb)();

          case 2:
            item = _schema.db.getSchema().table("Item");
            notNew = item.new.eq(false);
            newTaskQuery = _schema.db.select().from(item).where(item.new.eq(true));
            allQuery = _schema.db.select().from(item).where(notNew).orderBy(item.id, _lovefield.Order.ASC);
            activeQuery = _schema.db.select().from(item).where(_lovefield.op.and(notNew, item.complete.eq(false)));
            completedQuery = _schema.db.select().from(item).where(_lovefield.op.and(notNew, item.complete.eq(true)));

            // DOMVM

            empty = { newTask: {}, all: [], active: [], completed: [] };
            vm = _domvm2.default.createView({ render: _todoapp2.default }, empty).mount(document.getElementById("app"));


            _schema.db.observe(allQuery, updateDomvm);

            (0, _seed2.default)(_schema.db);
...
var _schema = __webpack_require__(124);

var _seed = __webpack_require__(332);

var _seed2 = _interopRequireDefault(_seed);

var _domvm = __webpack_require__(126);
...
exports.default = function () {
  var _ref = _asyncToGenerator( /*#__PURE__*/regeneratorRuntime.mark(function _callee(db) {
    var item, newTasks, tasks, rows;
    return regeneratorRuntime.wrap(function _callee$(_context) {
      while (1) {
        switch (_context.prev = _context.next) {
          case 0:
            item = db.getSchema().table("Item");
            newTasks = [{ id: 1, name: "Taste Javascript", edit: "", complete: true, editing: false, new: false }, { id: 2, name: "Buy a unicorn", edit: "", complete: false, editing: false, new: false }, { id: 3, name: "", edit: "", complete: false, editing: false, new: true }];
            _context.next = 4;
            return db.select().from(item).where(item.new.eq(true)).exec();

          case 4:
            tasks = _context.sent;
            rows = tasks.length > 0 ? [] : newTasks.map(function (r) {
              return item.createRow(r);
            });
            return _context.abrupt("return", db.insertOrReplace().into(item).values(rows).exec());
...
function TodoApp(vm, _ref) {
   var newTask = _ref.newTask,
       all = _ref.all,
       active = _ref.active,
       completed = _ref.completed;

   return el("section.todoapp", [Header(newTask), all.length == 0 ? [] : [Main(all, active.length > 0), Footer(active.length, completed.length > 0)]]);
}

function Header(_ref2) {
   var name = _ref2.name;

   return el("header.header", [el("h1", ["todos"]), el("form", { onsubmit: _controller.createTask }, [el("input.new-todo", { placeholder: "What needs to be done?",
      autofocus: true,
      value: name,
      onkeyup: _controller.setNewTaskName })])]);
}

function Main(tasks, anyActive) {
   return el("section.main", [el("input#toggle-all.toggle-all", { type: "checkbox",
      checked: !anyActive,
      onclick: (0, _controller.toggleAll)(anyActive) }), el("label", { for: "toggle-all" }, ["Mark all as complete"]), el("ul.todo-list", tasks.map(Todo))]);
}
...
function Todo(_ref3) {
   ...
   return el("li", { class: complete ? "completed" : null + editing ? "editing" : null }, [el("div.view", [el("input.toggle", { type: "checkbox",
      checked: complete,
      onchange: (0, _controller.toggleCompletion)(id) }), el("label", { ondblclick: (0, _controller.startEditing)(id) }, name), el("button.destroy", { onclick: (0, _controller.removeTask)(id) })]), el("input.edit", { value: edit,
      onkeyup: (0, _controller.editInput)(id),
      onblur: (0, _controller.blurEdit)(id) })]);
}
...
function Footer(activeCount, anyCompleted) {
   return el("footer.footer", [el("span.todo-count", [el("strong", activeCount), " ", pluralise(activeCount, "item"), " left"]),
   //el("ul.filters",
   //   [el("li",
   //       [el("a.selected",
   //           {href: "#/"},
   ...
   //           {href: "#/completed"},
   //           "Completed")])]),
   !anyCompleted ? null : el("button.clear-completed", { onclick: _controller.clearCompleted }, "Clear Completed")]);
}
...
exports.clearCompleted = exports.createTask = undefined;

// data: {"id": 2, "name": "Buy a unicorn", "complete": false}
var createTask = exports.createTask = function () {
  var _ref = _asyncToGenerator( /*#__PURE__*/regeneratorRuntime.mark(function _callee(event) {
    var item, tx, newItem, trimmedName, row;
    return regeneratorRuntime.wrap(function _callee$(_context) {
      while (1) {
        switch (_context.prev = _context.next) {
          case 0:
          ...
          case 5:
            _context.next = 7;
            return tx.attach(_schema.db.select().from(item).where(item.new.eq(true)));

          case 7:
            newItem = _context.sent;
            trimmedName = newItem[0].name.trim();
            ...
            _context.next = 12;
            return tx.attach(_schema.db.update(item).set(item.new, false).set(item.name, trimmedName).where(item.new.eq(true)));

          case 12:
            row = item.createRow({ name: "",
              edit: "",
              complete: false,
              editing: false,
              new: true });
            _context.next = 15;
            return tx.attach(_schema.db.insertOrReplace().into(item).values([row]));

          case 15:
            return _context.abrupt("return", tx.commit());
...
var clearCompleted = exports.clearCompleted = function () {
      ...
      while (1) {
        switch (_context5.prev = _context5.next) {
          case 0:
            item = _schema.db.getSchema().table("Item");
            _context5.next = 3;
            return _schema.db.delete().from(item).where(item.complete.eq(true)).exec();
  ...
  return function clearCompleted(_x5) {
...
exports.setNewTaskName = setNewTaskName;
...
exports.blurEdit = blurEdit;

var _schema = __webpack_require__(124);

function _asyncToGenerator(fn) { return function () { var gen = fn.apply(this, arguments); return new Promise(function (resolve, reject) { function step(key, arg) { try { var info = gen[key](arg); var value = info.value; } catch (error) { reject(error); return; } if (info.done) { resolve(value); } else { return Promise.resolve(value).then(function (value) { step("next", value); }, function (err) { step("throw", err); }); } } return step("next"); }); }; }

function setNewTaskName(event) {
  event.preventDefault();

  var name = event.target.value;
  var item = _schema.db.getSchema().table("Item");
  return _schema.db.update(item).set(item.name, name).where(item.new.eq(true)).exec();
}
        ...
        while (1) {
          switch (_context2.prev = _context2.next) {
            case 0:
              ...
              item = _schema.db.getSchema().table("Item");
              _context2.next = 4;
              return _schema.db.delete().from(item).where(item.id.eq(id)).exec();
      ...
      return regeneratorRuntime.wrap(function _callee3$(_context3) {
        while (1) {
          switch (_context3.prev = _context3.next) {
            ...
            case 6:
              togglingItems = _context3.sent;
              _context3.next = 9;
              return tx.attach(_schema.db.update(item).set(item.complete, !togglingItems[0].complete).where(item.id.eq(id)));

            case 9:
              return _context3.abrupt("return", tx.commit());
...
function toggleAll(to) {
  return function () {
    var _ref4 = _asyncToGenerator( /*#__PURE__*/regeneratorRuntime.mark(function _callee4(_event) {
      var item;
        ...
        while (1) {
          switch (_context4.prev = _context4.next) {
            case 0:
              item = _schema.db.getSchema().table("Item");
              _context4.next = 3;
              return _schema.db.update(item).set(item.complete, to).where(item.new.eq(false)).exec();
      ...
      return regeneratorRuntime.wrap(function _callee6$(_context6) {
        while (1) {
          switch (_context6.prev = _context6.next) {
            ...
            case 6:
              editingItem = _context6.sent;
              _context6.next = 9;
              return tx.attach(_schema.db.update(item).set(item.editing, false));

            case 9:
              _context6.next = 11;
              return tx.attach(_schema.db.update(item).set(item.editing, true).set(item.edit, editingItem[0].name).where(item.id.eq(id)));
...
function updateEdit(id, event) {
  ...
  var item = _schema.db.getSchema().table("Item");
  return _schema.db.update(item).set(item.edit, edit).where(item.id.eq(id)).exec();
}
